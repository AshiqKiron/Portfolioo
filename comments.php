...
<div id="comments" class="comments-area">
		...
		<h2 class="comments-title">
			<?php
				...
				if ( 1 === $comment_count ) {
					printf(
						/* translators: 1: title. */
						esc_html_e( 'One thought on &ldquo;%1$s&rdquo;', 'portfolioo' ),
						'<span>' . get_the_title() . '</span>'
					);
				} else {
					printf( // WPCS: XSS OK.
						/* translators: 1: comment count number, 2: title. */
						esc_html( _nx( '%1$s thought on &ldquo;%2$s&rdquo;', '%1$s thoughts on &ldquo;%2$s&rdquo;', $comment_count, 'comments title', 'portfolioo' ) ),
	...
	if ( ! comments_open() && get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) : ?>
		...
		<p class="no-comments"><?php esc_html_e( 'Comments are closed.', 'portfolioo' ); ?></p>
	<?php
	endif;
